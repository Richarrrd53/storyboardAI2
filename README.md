...
- **對稱性反向融化 (Reverse Stagger & Melt)**：
  收合不再是粗暴的全域瞬間隱藏，而是與展開形成對稱的「反向階梯融解 (Reverse Stagger)」：
  - 最底層「移至回收桶」（Child 5，0ms）率先溶解，接續 Divider（14ms）、匯出 JSON（26ms）、複製分鏡（38ms）、頂層重新命名（50ms），以每項 `12~14ms` 間隔反向漸隱；單一項目（回收桶還原）則保持 0ms 立即淡出。
  - 項目融出套用克制的柔和高斯模糊（`blur: 0 → 3px`、`opacity: 1 → 0`、`translateY: 0 → 3px`），營造文字重新融解蒸發回壓克力表面的精緻物理感。
- **0–40ms（Phase A1 選項反向融解）**：面板維持完整展開尺寸，子項目以反向階梯迅速融出。
- **40–180ms（Phase A2 表面收縮凝聚）**：當選項溶解約 60%~70% 時，面板平滑收束凝聚為 20px 實心小圓。
- **30–230ms（拋物線返程滑行）**：小圓沿反向弧線平滑滑回按鈕中心 $P_{0\text{-target}}$（即時取得觸發按鈕在視口中的最新座標，杜絕任何像素突跳；180–230ms 呈現純淨 20px 小圓滑進）。
- **230–440ms（Phase C 專屬 210ms 原位舒展與圖示重組）**：
...
- **空靈通透質感與髮絲接觸陰影**：
  - 半透壓克力基底：`background: rgba(255, 255, 255, 0.66)`。
  - 冷調微邊框：`border: 1px solid rgba(96, 120, 155, 0.14)`（取代純白厚重邊框，在淺色卡片上呈現極具質感的冷灰藍邊界分割）。
  - 超採樣毛玻璃霧化：`backdrop-filter: blur(16px) saturate(130%)`。
  - 髮絲接觸陰影與頂部環境光折射：`box-shadow: 0 2px 8px rgba(18, 32, 56, 0.06), inset 0 1px 0 rgba(255, 255, 255, 0.72)`。
  - 剔除深沉厚重的外部黑暈大投影，轉為緊貼面板的極細緻懸浮微陰影。
