...
class ApiSettings {
    ...
    effective(saved = {}) {
        const vertexAvailable = Boolean(this.env.GOOGLE_CLOUD_PROJECT_ID &&
            (this.env.GCP_SERVICE_ACCOUNT_BASE64 || (this.env.GOOGLE_APPLICATION_CREDENTIALS && fs.existsSync(this.env.GOOGLE_APPLICATION_CREDENTIALS))));
        return {
            youtubeApiKey: saved.youtubeApiKey || this.env.YOUTUBE_API_KEY || '',
    ...
    status(saved = {}) {
        ...
        return { configured: youtube && gemini, savedAt: saved.savedAt || null,
            youtube: { configured: youtube, source: saved.youtubeApiKey ? 'browser' : youtube ? 'server' : 'none' },
            gemini: { configured: gemini, source: saved.geminiApiKey ? 'browser' : gemini ? 'server' : 'none',
                mode: keys.geminiApiKey ? 'api-key' : keys.vertexAvailable ? 'vertex' : 'none' } };
    }
